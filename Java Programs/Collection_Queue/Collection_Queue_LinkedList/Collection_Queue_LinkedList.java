...
class Collection_Queue_LinkedList {
      public static void main(String args[]){  
      
        Queue<Student> QueueLinked_list = new LinkedList<Student>();

        System.out.println("QueueLinked_list.isEmpty(): "+QueueLinked_list.isEmpty());
        ...
        QueueLinked_list.add(st1);        
        QueueLinked_list.add(st2);
        QueueLinked_list.add(st3);         
        QueueLinked_list.add(st4); 
        QueueLinked_list.add(st5); 
        QueueLinked_list.add(st6); 
        
        int size = QueueLinked_list.size();
        System.out.println("List size : " + size);
                
        System.out.println("After Add: ");
        for(Student s : QueueLinked_list){
            System.out.println("id: "+ s.id + ", name: "+ s.name +", degree: " + s.Degree.course);
        }
        
        Student temp = new Student();
        temp = QueueLinked_list.peek(); 
        System.out.println("peek() Head : "+ temp.id+", "+temp.name);  

        temp = QueueLinked_list.poll(); 
        System.out.println("poll() Removed Head : "+ temp.id+", "+temp.name);    
        ...
        QueueLinked_list.clear(); //remove all elements 
