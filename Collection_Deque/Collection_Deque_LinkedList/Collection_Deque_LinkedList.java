...
class Collection_Deque_LinkedList {
    public static void main(String args[]){  

        Deque<Student> dq_Linked = new LinkedList<Student>();

        System.out.println("dq_Linked.isEmpty(): "+dq_Linked.isEmpty());
        ...
        dq_Linked.add(st2);
        dq_Linked.add(st3);        
        dq_Linked.addFirst(st4); // insert at start position
        dq_Linked.addLast(st5); //insert at end position        
        dq_Linked.push(st6); //insert at end position

        int size = dq_Linked.size();
        System.out.println("List size : " + size);
                
        System.out.println("After Add: ");
        for(Student s : dq_Linked){
            System.out.println("id: "+ s.id + ", name: "+ s.name +", degree: " + s.Degree.course);
        }

        Student temp = new Student();

        temp = dq_Linked.peek(); // return head
        System.out.println("Head : " + temp.id + ", "+ temp.name);        
        temp = dq_Linked.peekFirst(); //return first object
        System.out.println("First : " + temp.id + ", "+ temp.name);        
        temp = dq_Linked.peekLast(); //return last object
        System.out.println("Last : " + temp.id + ", "+ temp.name);        


        temp = dq_Linked.pop(); //remove first element
        System.out.println("pop : " + temp.id + ", "+ temp.name);   
        temp = dq_Linked.poll();      // remove the head   
        System.out.println("poll : " + temp.id + ", "+ temp.name);
        temp = dq_Linked.pollFirst(); //remove the first element
        System.out.println("pollFirst : " + temp.id + ", "+ temp.name);
        temp = dq_Linked.pollLast(); //remove last element
        System.out.println("pollLast : " + temp.id + ", "+ temp.name);

        temp = dq_Linked.removeFirst(); //remove first element
        System.out.println("removeFirst : " + temp.id + ", "+ temp.name);
        temp = dq_Linked.removeLast();  //remove last element
        System.out.println("removeLast : " + temp.id + ", "+ temp.name);
        dq_Linked.remove(st3);
        ...
        dq_Linked.clear(); //remove all elements 
