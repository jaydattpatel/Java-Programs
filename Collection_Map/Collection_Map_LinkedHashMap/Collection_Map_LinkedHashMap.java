...
class Collection_Map_LinkedHashMap {
    public static void main(String args[]){  

        Map<Integer,Student> map_Linkedhash = new LinkedHashMap<Integer,Student>();

        System.out.println("------map_Linkedhash.isEmpty(): "+map_Linkedhash.isEmpty());
        ...
        map_Linkedhash.put(st1.id, st1);        
        map_Linkedhash.put(st2.id, st2);
        map_Linkedhash.put(st3.id, st3);   
        map_Linkedhash.putIfAbsent(st4.id, st4);    
        map_Linkedhash.put(st5.id, st5);   
        map_Linkedhash.put(st6.id, st6);   

        int size = map_Linkedhash.size();
        System.out.println("---------List size : " + size);
                
        System.out.println("--------After Add: ");

        for (Map.Entry<Integer,Student> obj : map_Linkedhash.entrySet()) { 
            System.out.println(obj.getKey() + ":"+ obj.getValue().name +", "+ obj.getValue().Degree.course); 
        } 

        //get Set of keys
        Set<Integer> keys = new HashSet<Integer>();
        keys = map_Linkedhash.keySet(); 
        ...
        System.out.println("map_Linkedhash.containsKey(101) : "+map_Linkedhash.containsKey(101));        
        System.out.println("map_Linkedhash.containsValue(st5) : "+map_Linkedhash.containsValue(st5));

        map_Linkedhash.remove(103);        
        map_Linkedhash.remove(104,st4);

        Student st7 = new Student(106,"Bhavesh","LLB");        
        Student st8 = new Student(108,"Pratik","LLB");

        map_Linkedhash.replace(106, st7); // replace value by match with key.
        map_Linkedhash.replace(102, st2, st8); // replace value by match with key.
        ...
        map_Linkedhash.clear(); //remove all elements 
