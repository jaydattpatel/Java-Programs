...
class Collection_Map_LinkedHashMap {
    public static void main(String args[]){  
        ...
        Map<Integer,Student> map_tree = new TreeMap<Integer,Student>();

        System.out.println("------map_tree.isEmpty(): "+map_tree.isEmpty());
        ...
        map_tree.put(st1.id, st1);        
        map_tree.put(st2.id, st2);
        map_tree.put(st3.id, st3);   
        map_tree.putIfAbsent(st4.id, st4);    
        map_tree.put(st5.id, st5);   
        map_tree.put(st6.id, st6);   

        int size = map_tree.size();
        System.out.println("---------List size : " + size);
                
        System.out.println("--------After Add: ");

        for (Map.Entry<Integer,Student> obj : map_tree.entrySet()) { 
            System.out.println(obj.getKey() + ":"+ obj.getValue().name +", "+ obj.getValue().Degree.course); 
        } 

        //get Set of keys
        Set<Integer> keys = new HashSet<Integer>();
        keys = map_tree.keySet(); 
        ...
        System.out.println("map_tree.containsKey(101) : "+map_tree.containsKey(101));        
        System.out.println("map_tree.containsValue(st5) : "+map_tree.containsValue(st5));

        map_tree.remove(103);        
        map_tree.remove(104,st4);

        Student st7 = new Student(106,"Bhavesh","LLB");        
        Student st8 = new Student(108,"Pratik","LLB");

        map_tree.replace(106, st7); // replace value by match with key.
        map_tree.replace(102, st2, st8); // replace value by match with key.
        ...
        map_tree.clear(); //remove all elements 
